...
import { update_banque_action } from '@/actions/banque_action'
...
export default function EditBanque({ banque }: { banque: Banque }) {
   ...
   const form = useForm<z.infer<typeof banqueSchema>>({
      ...
      defaultValues: {
         nom_banque: banque.nom_bank,
         numero_compte_banque: banque.numero_compte,
      },
   })
   const onSubmit = (values: z.infer<typeof banqueSchema>) => {
      startTransition(() => {
         ...
         update_banque_action(values, banque.id).then((data) => {
            if (data.success) {
